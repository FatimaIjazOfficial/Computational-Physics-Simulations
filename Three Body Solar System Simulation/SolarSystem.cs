    ...
    public class SolarSystem
    {
        ...
        public void DrawThreeBodyWithoutArray(Form1 form)
        {
            ...
            for (int i = 0; i < size - 1; i++)
            {
                ...
                Vxe = Vxe - 4 * pi * pi * xe * dt / Math.Pow(re, beta + 1) - 4 * pi * pi * mjms * (xj - xe) * dt / Math.Pow(rej, beta + 1);
                Vye = Vye - 4 * pi * pi * ye * dt / Math.Pow(re, beta + 1) - 4 * pi * pi * mjms * (yj - ye) * dt / Math.Pow(rej, beta + 1);
                Vxj = Vxj - 4 * pi * pi * xj * dt / Math.Pow(rj, beta + 1) - 4 * pi * pi * mems * (xj - xe) * dt / Math.Pow(rej, beta + 1);
                Vyj = Vyj - 4 * pi * pi * yj * dt / Math.Pow(rj, beta + 1) - 4 * pi * pi * mems * (yj - ye) * dt / Math.Pow(rej, beta + 1);
